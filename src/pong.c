/* http://srecord.sourceforge.net/ */
const int pongGame[] =
{ ,
};
const unsigned long pongGame_termination = 0x00000000;
const unsigned long pongGame_start       = 0x00000000;
const unsigned long pongGame_finish      = 0x000000F6;
const int pongGame_length      = 0x000000F6;

#define PONGGAME_TERMINATION 0x00000000
#define PONGGAME_START       0x00000000
#define PONGGAME_FINISH      0x000000F6
#define PONGGAME_LENGTH      0x000000F6
