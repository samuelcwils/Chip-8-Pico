/* http://srecord.sourceforge.net/ */
const unsigned char breakout[] =
{
,
};
const unsigned long breakout_termination = 0x00000000;
const unsigned long breakout_start       = 0x00000000;
const unsigned long breakout_finish      = 0x00000118;
const unsigned long breakout_length      = 0x00000118;

#define BREAKOUT_TERMINATION 0x00000000
#define BREAKOUT_START       0x00000000
#define BREAKOUT_FINISH      0x00000118
#define BREAKOUT_LENGTH      0x00000118
